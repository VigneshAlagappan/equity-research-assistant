...
| High | Investor Relations + document pipeline, full version (README [step 7](README.md#implementation-sequence)) | 🟡 partial | `research/documents.py` still covers the Q&A/Signals evidence path with direct PDF extraction per question, no cache. Page-scoped chunking + FTS5 keyword search now exist as their own capability (Step 2D, see [architecture.md's Document Retrieval](architecture.md#document-retrieval-retrievaldocument_searchpy-step-2d)) but are deliberately not wired into that evidence path or the Commentary tab yet; no `hybrid_search` module exists. Still missing: the Commentary tab's attributed pull-quote extraction. |
...
loop, Critical/red tier, the remaining 8 of 10 planned rule families,
