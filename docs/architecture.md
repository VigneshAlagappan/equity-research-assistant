...
| `companies/` | Company registry (`registry.py` — country/currency/fiscal-year-end are per-company, not global), lifecycle/archive rules (`lifecycle.py`), NSE bulk-import (`nse_import.py`, India-only), the US-ticker collision-resolution/normalization helpers (`us_universe.py` — `resolve_us_company_id()`, `normalize_us_ticker()`, `existing_us_company_id()`, shared by `scripts/register_sp500_companies.py` and `scripts/register_russell3000_companies.py`, see Known gaps for the bulk-importer note this replaces), discrete stock-action records (`stock_actions.py` — splits/bonus/rights issues, raw events only). |
...
| `scripts/` | One-off/bulk scripts: data-workbook imports (`import_*.py`, `parse_equity_analysis_workbook.py`), NSE/SEC EDGAR/FRED batch fetchers (`batch_fetch_nse.py`, `batch_fetch_sec_edgar.py`, `batch_fetch_fred.py`, `fetch_nse_xbrl.py`, `fetch_nse_shareholding.py`, `xbrl_diagnostic.py`), US company-universe registration (`register_sp500_companies.py`, `register_russell3000_companies.py` — the latter registers/tags Russell 3000+1000+2000 membership from a consolidated iShares-holdings-derived workbook or per-ETF CSVs, with point-in-time reconstitution via `company_index_membership`'s provenance columns, see Data model below), backfills (`backfill_company_websites.py`, `backfill_sector_industry.py`, `backfill_price_history.py`/`backfill_price_history_usa.py` — 20-year target depth reached incrementally across scheduled runs via a per-tier time budget, and per-tier scoping, see USER_GUIDE.md §12), the daily price jobs (`fetch_daily_prices.py`/`fetch_daily_prices_usa.py`), the raw-object catalog reconciliation job (`reconcile_raw_objects.py`, ADR-022), and db sharding (`db_shard.py`/`db_unshard.py` — see [USER_GUIDE.md](USER_GUIDE.md#13-database-sharding-git-storage)). |
...
| `watchlist.html` | Grid/Feed activity workspace (2026-09-26 redesign) — a company rail (All + per-company, `?company=` shareable) plus Grid (one panel per type) and Feed (single day-grouped list) views over one merged activity model built by `web/watchlist_feed.py::list_watchlist_activity()`. News (`company_news`, live-refetched per watchlisted company on load) and Investigations (`research_cases`/`investigation_companies`) are real; Announcements and Financial & filing-updates panels render an honest empty state pending the ingestion pipeline in `docs/pendingList.md`. Client-side Grid/Feed toggle and rail-collapse state (no reload), persisted to `localStorage`. |
...
- **Bulk US company-master importer (resolved 2026-09-26)** — `companies/
  nse_import.py` bulk-registers from an NSE export; the US parallel is now
  `scripts/register_sp500_companies.py` (S&P 500) and `scripts/
  register_russell3000_companies.py` (Russell 3000/1000/2000, from a
  consolidated iShares-holdings workbook or per-ETF CSVs — 2,043 companies
  registered/tagged in the first full run). Both share collision-resolution/
  normalization logic via `companies/us_universe.py`. `company_index_
  membership` gained `source`/`retrieved_at`/`effective_from`/`effective_to`/
  `status` columns to support point-in-time reconstitution (a company
  dropping out of an index is marked `status='historical'`, not deleted).
  SEC EDGAR annual-financials and price-history backfill for this new
  universe (`batch_fetch_sec_edgar.py --index "Russell 3000"`,
  `backfill_price_history_usa.py`) have **not** been run yet — registration/
  tagging only so far.
