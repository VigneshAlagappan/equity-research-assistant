...
| Watchlist | Grid/Feed activity workspace | 🟡 | Company rail (All + per-company, `?company=` shareable) plus Grid (one panel per type) and Feed (single newest-first list, grouped Today/Yesterday/Earlier) views over one merged activity model (`web/watchlist_feed.py::list_watchlist_activity()`), client-side view/filter toggling (no reload), rail-open and Grid/Feed state persisted to `localStorage`. News (real, from `company_news`, live-refetched for every watchlisted company on each page load, 7-day window) and Investigations (real, from `research_cases`/`investigation_companies`, in_progress/completed/failed status pills) are populated with live data. Partial only because the Announcements and Financial & filing-updates panels render their real empty state ("not yet available") — no ingestion source exists yet; see `docs/pendingList.md`'s "Watchlist Announcements & Financial-filing ingestion pipeline" for the deferred backend spec. The old per-company lazy "Latest news (24h)" disclosure this replaced is gone from this page (the underlying `/watchlist/news/<id>` endpoint still serves the Company page's own Overview-tab news section). |
