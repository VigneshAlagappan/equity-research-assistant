...
- ADR-021's decision that `financial_observations` stays SQLite-only, and the recommendation in this same file (see "Known bug, NOT fixed") about the Postgres-port gap there, are untouched by this ADR — this refactor is about the **raw artifact layer**, not a re-litigation of where reconciled/canonical values live.
...
- **Do not conflate this with the still-unfixed `financial_observations`/Postgres gap** (documented above, under ADR-021's "Known bug, NOT fixed" section) — that gap should likely be resolved, or at least its resolution path decided, before or alongside adding a lineage column that would point at rows in that same table.
